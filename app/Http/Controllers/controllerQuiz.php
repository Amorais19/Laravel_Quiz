...
class controllerQuiz extends Controller{
    ...
    public function dadosPagina5(Request $request){
        ...
        if($acertos == 10)
            $dados['mensagem'] = "Parabéns! Você é um baita fã de My Little Pony.";
        else if($acertos > 5)
            $dados['mensagem'] = "Muito bom! Você é um bom fã de My Little Pony.";
        else if($acertos = 5)
            $dados['mensagem'] = "Que legal! Você é fã de My Little Pony.";
        else if($acertos > 3)
            $dados['mensagem'] = "Bacana! Mas talvez você não seja tão fã de My Little Pony.";
        else
            $dados['mensagem'] = "Ruim. Acho que você não é fã de My Little Pony.";

        return view('result', compact('dados'));
    }
}
// falta só arrumar as mensagens